...
namespace front_to_front_relaxation_heuristic {
...
FrontToFrontRelaxationHeuristic::FrontToFrontRelaxationHeuristic(
  ...
  for (OperatorProxy op : task_proxy.get_operators()) build_unary_operators(op);
...
bool FrontToFrontRelaxationHeuristic::dead_ends_are_reliable() const {
  return !task_properties::has_axioms(task_proxy);
}

PropID FrontToFrontRelaxationHeuristic::get_prop_id(int var, int value) const {
  return proposition_offsets[var] + value;
}

PropID FrontToFrontRelaxationHeuristic::get_prop_id(
    const FactProxy &fact) const {
  return get_prop_id(fact.get_variable().get_id(), fact.get_value());
}

const Proposition *FrontToFrontRelaxationHeuristic::get_proposition(
    int var, int value) const {
  return &propositions[get_prop_id(var, value)];
}

Proposition *FrontToFrontRelaxationHeuristic::get_proposition(int var,
                                                              int value) {
  return &propositions[get_prop_id(var, value)];
}

Proposition *FrontToFrontRelaxationHeuristic::get_proposition(
    const FactProxy &fact) {
  return get_proposition(fact.get_variable().get_id(), fact.get_value());
}

void FrontToFrontRelaxationHeuristic::build_unary_operators(
  ...
  for (EffectProxy effect : op.get_effects()) {
    ...
    unary_operators.emplace_back(preconditions_copy.size(), precond_index,
                                 effect_prop, op_no, base_cost);
    precondition_props.erase(precondition_props.end() - eff_conds.size(),
                             precondition_props.end());
  }
}

void FrontToFrontRelaxationHeuristic::simplify() {
  /*
  ...
  */
  ...
  cout << "Simplifying " << unary_operators.size() << " unary operators..."
       << flush;
    ...
    */
    ...
    for (int mask = 0; mask < powerset_size; ++mask) {
      ...
      for (size_t i = 0; i < precondition.size(); ++i)
        if (mask & (1 << i)) dominating_precondition.push_back(precondition[i]);
      ...
      if (found != unary_operator_index.end()) {
        ...
        if (dominator_cost <= cost) return true;
      }
    }
    return false;
  };
...
void FrontToFrontRelaxationHeuristic::do_set_goal() {
...
void FrontToFrontRelaxationHeuristic::set_original_goal() {
  for (PropID prop_id : goal_propositions)
    propositions[prop_id].is_goal = false;
