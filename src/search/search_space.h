...
class SearchNode {
 ...
 public:
  SearchNode(const StateRegistry &state_registry, StateID state_id,
             SearchNodeInfo &info);

  StateID get_state_id() const { return state_id; }
  GlobalState get_state() const;

  StateID get_parent_state_id() const { return info.parent_state_id; }
  ...
  void open_initial();
  void open(const SearchNode &parent_node, const OperatorProxy &parent_op,
            int adjusted_cost);
  void reopen(const SearchNode &parent_node, const OperatorProxy &parent_op,
              int adjusted_cost);
