...
namespace backward_eager_search {
BackwardEagerSearch::BackwardEagerSearch(const Options &opts)
    : SearchEngine(opts),
...
void BackwardEagerSearch::initialize() {
  ...
  */
  ...
  /*
    ...
    operator.
  */
  EvaluationContext eval_context(global_goal_state, 0, true, &statistics);
  ...
  if (open_list->is_dead_end(eval_context)) {
    ...
    SearchNode node = search_space.get_node(global_goal_state);
    node.open_initial();
  ...
  pruning_method->initialize(inverse_task);
}

void BackwardEagerSearch::print_statistics() const {
  statistics.print_detailed_statistics();
  search_space.print_statistics();
  pruning_method->print_statistics();
}

SearchStatus BackwardEagerSearch::step() {
  ...
  if (task_properties::is_goal_state(inverse_task_proxy, s)) {
  ...
  */
  ...
  for (OperatorID op_id : applicable_ops) {
    OperatorProxy op = inverse_task_proxy.get_operators()[op_id];
...
void BackwardEagerSearch::reward_progress() {
  // Boost the "preferred operator" open lists somewhat whenever
  // one of the heuristics finds a state with a new best h value.
  open_list->boost_preferred();
}

void BackwardEagerSearch::dump_search_space() const {
  search_space.dump(inverse_task_proxy);
}

void BackwardEagerSearch::start_f_value_statistics(
...
/* TODO: HACK! This is very inefficient for simply looking up an h value.
   ...
   state. */
void BackwardEagerSearch::update_f_value_statistics(
    EvaluationContext &eval_context) {
  if (f_evaluator) {
    int f_value = eval_context.get_evaluator_value(f_evaluator.get());
    statistics.report_f_value_progress(f_value);
