...
class RootTask : public AbstractTask {
  ...
  const ExplicitEffect &get_effect(int op_id, int effect_id,
                                   bool is_axiom) const;
...
static void check_facts(const vector<FactPair> &facts,
                        const vector<ExplicitVariable> &variables) {
...
static void check_facts(const ExplicitOperator &action,
                        const vector<ExplicitVariable> &variables) {
...
ExplicitEffect::ExplicitEffect(int var, int value,
                               vector<FactPair> &&conditions)
...
const ExplicitEffect &RootTask::get_effect(int op_id, int effect_id,
                                           bool is_axiom) const {
...
const ExplicitOperator &RootTask::get_operator_or_axiom(int index,
                                                        bool is_axiom) const {
  if (is_axiom) {
...
int RootTask::get_num_variables() const { return variables.size(); }
...
bool RootTask::are_facts_mutex(const FactPair &fact1,
...
int RootTask::get_num_operators() const { return operators.size(); }
...
FactPair RootTask::get_operator_precondition(int op_index, int fact_index,
                                             bool is_axiom) const {
...
FactPair RootTask::get_operator_effect_condition(int op_index, int eff_index,
                                                 int cond_index,
                                                 bool is_axiom) const {
...
FactPair RootTask::get_operator_effect(int op_index, int eff_index,
                                       bool is_axiom) const {
  return get_effect(op_index, eff_index, is_axiom).fact;
}

int RootTask::convert_operator_index(int index,
...
int RootTask::get_num_axioms() const { return axioms.size(); }

int RootTask::get_num_goals() const { return goals.size(); }
...
void RootTask::convert_state_values(vector<int> &,
                                    const AbstractTask *ancestor_task) const {
  if (this != ancestor_task) {
