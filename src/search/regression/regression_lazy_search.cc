...
RegressionLazySearch::RegressionLazySearch(const Options &opts)
    : SearchEngine(opts),
      ...
      current_state(regression_state_registry.create_goal_state(
...
void RegressionLazySearch::initialize() {
...
vector<OperatorID> RegressionLazySearch::get_successor_operators(
    const ordered_set::OrderedSet<OperatorID> &preferred_operators) const {
  vector<OperatorID> applicable_operators;
  regression_successor_generator.generate_applicable_ops(current_state,
...
void RegressionLazySearch::generate_successors() {
  ...
  for (OperatorID op_id : successor_operators) {
    ...
    if (new_real_g < bound) {
      EvaluationContext new_eval_context(current_eval_context.get_cache(),
                                         new_g, is_preferred, nullptr);
      open_list->insert(new_eval_context,
                        make_pair(current_state.get_id(), op_id));
    }
  }
}

SearchStatus RegressionLazySearch::fetch_next_state() {
  StateID current_state_id = StateID::no_state;

  while (current_state_id == StateID::no_state) {
    if (open_list->empty()) {
    ...
    GlobalState current_predecessor =
        regression_state_registry.lookup_state(current_predecessor_id);
  ...
  current_state = regression_state_registry.lookup_state(current_state_id);
  ...
  */
  open_list->set_goal(current_state);
  current_eval_context =
      EvaluationContext(regression_state_registry.get_initial_state(),
                        current_g, true, &statistics);

  return IN_PROGRESS;
}

SearchStatus RegressionLazySearch::step() {
  // Invariants:
  ...
  if (node.is_new() || reopen) {
    if (current_operator_id != OperatorID::no_operator) {
      ...
      GlobalState parent_state =
          regression_state_registry.lookup_state(current_predecessor_id);
      for (Evaluator *evaluator : path_dependent_evaluators)
        evaluator->notify_state_transition(current_state, current_operator_id,
                                           parent_state);
    }
    ...
    if (!open_list->is_dead_end(current_eval_context)) {
      ...
      node.close();
      if (check_initial_and_set_plan(current_state)) return SOLVED;
      if (search_progress.check_progress(current_eval_context)) {
        statistics.print_checkpoint_line(current_g);
        reward_progress();
      }
...
void RegressionLazySearch::reward_progress() { open_list->boost_preferred(); }

void RegressionLazySearch::print_statistics() const {
  statistics.print_detailed_statistics();
  partial_state_search_space.print_statistics();
}

bool RegressionLazySearch::check_initial_and_set_plan(
