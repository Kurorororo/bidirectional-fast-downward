...
struct ExProposition {
  ...
  bool operator<(const ExProposition &other) const { return fact < other.fact; }
};
...
class Exploration {
 ...
 public:
  ...
  void compute_reachability_with_excludes(
      ...
      std::vector<utils::HashMap<FactPair, int>> &lvl_op, bool level_out,
      const std::vector<FactPair> &excluded_props,
      const std::unordered_set<int> &excluded_op_ids, bool compute_lvl_ops);
