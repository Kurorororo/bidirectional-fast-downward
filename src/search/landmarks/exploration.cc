...
/* Integration Note: this class is the same as (rich man's) FF heuristic
   ...
   - Added-on functionality for excluding certain operators from the relaxed
     ...
     computation)
   - Exploration can be done either using h_max or h_add criterion (h_max is
   needed during landmark generation, h_add later for heuristic computations)
   ...
   - Added-on functionality to check for a set of propositions which one is
   reached first by the relaxed exploration, and extract preferred operators for
   this cheapest proposition (needed for planning to nearest landmark).
*/
...
Exploration::Exploration(const TaskProxy &task_proxy)
  ...
  for (OperatorProxy op : axioms) build_unary_operators(op);
...
void Exploration::build_unary_operators(const OperatorProxy &op) {
  ...
  for (EffectProxy effect : op.get_effects()) {
    ...
    ExProposition *effect_proposition =
        &propositions[effect_fact.get_variable().get_id()]
                     [effect_fact.get_value()];
    int op_or_axiom_id = get_operator_or_axiom_id(op);
    unary_operators.emplace_back(precondition, effect_proposition,
                                 op_or_axiom_id, base_cost);
    precondition.clear();
...
void Exploration::setup_exploration_queue(
  ...
  for (ExUnaryOperator &op : unary_operators) {
    ...
    if (!excluded_op_ids.empty() &&
        (op.effect->h_add_cost == -2 ||
         excluded_op_ids.count(op.op_or_axiom_id))) {
      op.h_add_cost =
          -2;  // operator will not be applied during relaxed exploration
      continue;
    }
...
void Exploration::relaxed_exploration(bool use_h_max, bool level_out) {
  ...
  while (!prop_queue.empty()) {
    ...
    if (!level_out && prop->is_termination_condition && --unsolved_goals == 0)
      return;
    const vector<ExUnaryOperator *> &triggered_operators =
    ...
    for (size_t i = 0; i < triggered_operators.size(); ++i) {
      ...
      unary_op->h_max_cost =
          max(prop_cost + unary_op->base_cost, unary_op->h_max_cost);
      ...
      if (unary_op->unsatisfied_preconditions == 0) {
        int depth = unary_op->is_induced_by_axiom(task_proxy)
                        ...
                        : unary_op->depth + 1;
        if (use_h_max)
          enqueue_if_necessary(unary_op->effect, unary_op->h_max_cost, depth,
                               unary_op, use_h_max);
        else
          enqueue_if_necessary(unary_op->effect, unary_op->h_add_cost, depth,
                               unary_op, use_h_max);
      }
    }
  }
...
void Exploration::enqueue_if_necessary(ExProposition *prop, int cost, int depth,
                                       ExUnaryOperator *op, bool use_h_max) {
  assert(cost >= 0);
  if (use_h_max && (prop->h_max_cost == -1 || prop->h_max_cost > cost)) {
    ...
    prop_queue.push(cost, prop);
             ...
             (prop->h_add_cost == -1 || prop->h_add_cost > cost)) {
...
void Exploration::compute_reachability_with_excludes(
    ...
    bool level_out, const vector<FactPair> &excluded_props,
    const unordered_set<int> &excluded_op_ids, bool compute_lvl_ops) {
  // Perform exploration using h_max-values
  setup_exploration_queue(task_proxy.get_initial_state(), excluded_props,
                          excluded_op_ids, true);
  ...
  for (size_t var_id = 0; var_id < propositions.size(); ++var_id) {
    for (size_t value = 0; value < propositions[var_id].size(); ++value) {
      ExProposition &prop = propositions[var_id][value];
      if (prop.h_max_cost >= 0) lvl_var[var_id][value] = prop.h_max_cost;
    }
  }
  if (compute_lvl_ops) {
    for (ExUnaryOperator &op : unary_operators) {
      ...
      if (op.h_max_cost == numeric_limits<int>::max()) break;
      ...
      // If we have found a cheaper achieving operator, adjust h_max cost of
      // proposition.
...
bool Exploration::compute_reachability() {
  vector<FactPair> dummy_facts;
  unordered_set<int> dummy_ops;
  setup_exploration_queue(task_proxy.get_initial_state(), dummy_facts,
                          dummy_ops, true);
  relaxed_exploration(true, true);

  for (auto prop : goal_propositions)
    if (prop->h_max_cost == -1) return false;
