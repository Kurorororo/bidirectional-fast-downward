  ...
  if (evals.size() == 1 && preferred_evaluators.empty()) {
    ...
    for (const shared_ptr<Evaluator> &evaluator : evals) {
      subfactories.push_back(
          create_standard_scalar_open_list_factory(evaluator, false));
      if (!preferred_evaluators.empty()) {
        subfactories.push_back(
            create_standard_scalar_open_list_factory(evaluator, true));
      }
    }
...
shared_ptr<OpenListFactory> create_wastar_open_list_factory(
  ...
  return create_alternation_open_list_factory_aux(
      f_evals, options.get_list<shared_ptr<Evaluator>>("preferred"),
      options.get<int>("boost"));
}
...
create_astar_open_list_factory_and_f_eval(const Options &opts) {
  ...
  shared_ptr<Evaluator> f =
      make_shared<SumEval>(vector<shared_ptr<Evaluator>>({g, h}));
...
pair<shared_ptr<OpenListFactory>, shared_ptr<OpenListFactory>>
create_bidirectional_greedy_open_list_factory(const Options &options) {
  auto open_f = create_alternation_open_list_factory_aux(
      options.get_list<shared_ptr<Evaluator>>("evals_f"),
      options.get_list<shared_ptr<Evaluator>>("preferred_f"),
      options.get<int>("boost"));
  auto open_b = create_alternation_open_list_factory_aux(
      options.get_list<shared_ptr<Evaluator>>("evals_b"),
      options.get_list<shared_ptr<Evaluator>>("preferred_b"),
      options.get<int>("boost"));

  return make_pair(open_f, open_b);
}
}  // namespace search_common
